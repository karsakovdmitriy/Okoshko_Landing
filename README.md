...
Вам понадобится переменная `DATABASE_URL`. **Внимание:** она обязательно должна начинаться с протокола `postgresql://` или `postgres://`.
...
Если вы видите эту ошибку в логах Vercel при попытке сохранить контакт:

1.  **Проверьте Port:** Если вы используете Supabase, убедитесь, что в `DATABASE_URL` указан порт **6543** (Pooler), а не 5432. Для Serverless-функций (Vercel) использование пулера обязательно.
2.  **Transaction Mode:** В настройках Supabase выберите режим **Transaction mode** для строки подключения.
3.  **Белый список IP:** Убедитесь, что ваша база данных разрешает подключения со всех IP (0.0.0.0/0), так как у Vercel динамические IP-адреса.
4.  **Синхронизация схемы:** Проверьте, что вы выполнили команду `npx prisma db push`. Без неё таблицы в базе просто не существует.
